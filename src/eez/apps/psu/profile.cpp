        ...
        if (persist_conf::loadProfile(location, &profile) && profile.flags.isValid) {
            if (persist_conf::saveProfile(0, &profile)) {
                if (recallFromProfile(&profile, location)) {
                    event_queue::pushEvent(event_queue::EVENT_INFO_RECALL_FROM_PROFILE_0 +
                                           location);
                    return true;
                } else {
                    return false;
...
void getSaveName(const Parameters *profile, char *name) {
    if (!profile->flags.isValid ||
        strncmp(profile->name, AUTO_NAME_PREFIX, strlen(AUTO_NAME_PREFIX)) == 0) {
...
void fillProfile(Parameters *pProfile, int location, const char *name,
                 const Parameters *pCurrentProfile) {
        ...
        if (i < CH_NUM) {
            ...
            if (location != 0 || list::getListsChanged(channel)) {
                ...
                if (pCurrentProfile && currentProfile.flags.isValid) {
                    profile.channels[i].flags.listSaved =
                        currentProfile.channels[i].flags.listSaved;
                } else {
...
bool saveToFile(const char *filePath, int *err) {
    ...
    profile.header.checksum =
        persist_conf::calc_checksum((const persist_conf::BlockHeader *)&profile, sizeof(profile));
