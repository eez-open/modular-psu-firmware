...
int reg_get_ques_isum_bit_mask_for_channel_protection_value(Channel *channel,
                                                            ProtectionValue &cpv) {
...
void reg_set_ques_isum_bit(scpi_t *context, Channel *channel, int bit_mask, bool on) {
    scpi_psu_reg_name_t reg_name = channel->index == 1 ? SCPI_PSU_CH_REG_QUES_INST_ISUM_COND1
    ...
    if (on) {
        if (!(val & bit_mask)) {
            ...
            // set event on raising condition
            reg_name = channel->index == 1 ? SCPI_PSU_CH_REG_QUES_INST_ISUM_EVENT1
...
void reg_set_oper_isum_bit(scpi_t *context, Channel *channel, int bit_mask, bool on) {
    scpi_psu_reg_name_t reg_name = channel->index == 1 ? SCPI_PSU_CH_REG_OPER_INST_ISUM_COND1
    ...
    if (on) {
        if (!(val & bit_mask)) {
            ...
            // set event on raising condition
            reg_name = channel->index == 1 ? SCPI_PSU_CH_REG_OPER_INST_ISUM_EVENT1
