...
bool setType(Type value) {
    if (g_channelCoupling != value) {
        if (value != TYPE_NONE && !isCouplingOrTrackingAllowed(value)) {
            return false;
        }
        ...
        bp3c::relays::switchChannelCoupling(g_channelCoupling);
        ...
        } else if (isCommonGround()) {
            event_queue::pushEvent(event_queue::EVENT_INFO_COUPLED_IN_COMMON_GROUND);
        } else if (isSplitRail()) {
            event_queue::pushEvent(event_queue::EVENT_INFO_COUPLED_IN_SPLIT_RAIL);
        } else if (isTracked()) {
        ...
        setOperBits(OPER_GROUP_COMMON_GROUND, isCommonGround());
        setOperBits(OPER_GROUP_SPLIT_RAIL, isSplitRail());
