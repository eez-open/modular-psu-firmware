...
float remapAdcDataToVoltage(Channel& channel, AdcDataType adcDataType, int16_t adcData) {
    float value = remap((float)adcData, (float)AnalogDigitalConverter::ADC_MIN, channel.U_MIN, (float)AnalogDigitalConverter::ADC_MAX, channel.U_MAX_CONF);
#if !defined(EEZ_PLATFORM_SIMULATOR)    
    if (adcDataType == ADC_DATA_TYPE_U_MON || !channel.flags.rprogEnabled) {
        value -= channel.VOLTAGE_GND_OFFSET;
    }
#endif
    return value;
}
...
float remapAdcDataToCurrent(Channel& channel, AdcDataType adcDataType, int16_t adcData) {
    float value = remap((float)adcData, (float)AnalogDigitalConverter::ADC_MIN, channel.I_MIN, (float)AnalogDigitalConverter::ADC_MAX, channel.getDualRangeMax());
#if !defined(EEZ_PLATFORM_SIMULATOR)    
