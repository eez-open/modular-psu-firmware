...
void drawScale(const Widget *widget, const ScaleWidget *scale_widget, const Style *style,
               int y_from, int y_to, int y_min, int y_max, int y_value, int f, int d,
               bool drawTicks) {
    ...
    for (int y_i = y_from; y_i <= y_to; ++y_i) {
        ...
        if (drawTicks) {
            ...
            if (y_i >= y_min && y_i <= y_max) {
                if (y_i % s == 0) {
                    ...
                    if (vertical) {
                        display::drawHLine(x1, y, l1 - 1);
                    } else {
                        display::drawVLine(y, x1, l1 - 1);
                    }
                } else if (y_i % (s / 2) == 0) {
                    display::setColor(style->border_color);
                    if (vertical) {
                        if (flip) {
                            display::drawHLine(x1 + l1 / 2, y, l1 / 2);
                        } else {
                            display::drawHLine(x1, y, l1 / 2);
                        }
                    } else {
                        if (flip) {
                            display::drawVLine(y, x1 + l1 / 2, l1 / 2);
                        } else {
                            display::drawVLine(y, x1, l1 / 2);
                        }
                    }
                } else if (y_i % (s / 10) == 0) {
                    display::setColor(style->border_color);
                    if (vertical) {
                        if (flip) {
                            display::drawHLine(x1 + l1 - l1 / 4, y, l1 / 4);
                        } else {
                            display::drawHLine(x1, y, l1 / 4);
                        }
                    } else {
                        if (flip) {
                            display::drawVLine(y, x1 + l1 - l1 / 4, l1 / 4);
                        } else {
                            display::drawVLine(y, x1, l1 / 4);
                        }
                    }
                } else {
        ...
        if (d <= int(needleSize / 2)) {
            ...
            if (vertical) {
                if (flip) {
                    display::drawHLine(x2, y, l2 - d - 1);
                } else {
                    display::drawHLine(x2 + d, y, l2 - d - 1);
                }
            } else {
                if (flip) {
                    display::drawVLine(y, x2, l2 - d - 1);
                } else {
                    display::drawVLine(y, x2 + d, l2 - d - 1);
                }
            }
            ...
            if (y_i != y_value) {
                ...
                if (vertical) {
                    if (flip) {
                        display::drawHLine(x2 + l2 - d, y, d - 1);
                    } else {
                        display::drawHLine(x2, y, d - 1);
                    }
                } else {
                    if (flip) {
                        display::drawVLine(y, x2 + l2 - d, d - 1);
                    } else {
                        display::drawVLine(y, x2, d - 1);
                    }
                }
            }
        } else {
            ...
            display::setColor(style->background_color);
            if (vertical) {
                display::drawHLine(x2, y, l2 - 1);
            } else {
                display::drawVLine(y, x2, l2 - 1);
            }
        }
    }
}
...
void ScaleWidget_draw(const WidgetCursor &widgetCursor) {
    ...
    if (refresh) {
        ...
        drawScale(widget, scale_widget, style, y_from_min, y_from_max, y_min, y_max, y_value, f, d,
                  true);
