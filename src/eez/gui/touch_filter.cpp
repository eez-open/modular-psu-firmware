...
template <int N, int D> class IIRFilter {
  public:
    IIRFilter() : s(0) {
    }
...
template <int N, int D> class ChannelFilter {
  public:
    int operator()(int x, bool reset = false) {
        return i(m(x), reset);
    }
...
template <int P> class DebounceFilter {
  public:
    DebounceFilter() : s(0) {
    }
...
template <int N, int D, int P> class SampleFilter {
  public:
...
typedef SampleFilter<CONF_GUI_TOUCH_FILTER_N, CONF_GUI_TOUCH_FILTER_D, CONF_GUI_TOUCH_FILTER_P>
    DefaultSampleFilter;
...
class ScreenTransform {
  ...
  private:
    int ch_scale(long adc_min, long adc_range, long dim, long x) const {
        if (adc_range) {
            x = ((x - adc_min) * (dim - margin * 2) + (adc_range / 2)) / adc_range + margin;
...
bool calibrateTransform(int tl_x, int tl_y, int br_x, int br_y, int tr_x, int tr_y, int margin,
    ...
    return g_transform.calibrate(Point(tl_x, tl_y), Point(br_x, br_y), Point(tr_x, tr_y), margin,
                                 Point(width, height));
}
